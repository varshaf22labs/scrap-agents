...
ENTRY_URL = 'https://www.mongodb.com/company/careers'
DEFAULT_OUTPUT = 'output\\jobs.jsonl'
PAGINATION = {'type': 'button', 'text': 'alert beware of mongodb impersonators offering job opportunities learn more > beware of mongodb impersonators offering j', 'selector': 'a[class~="css-6prp03"]', 'href': 'https://www.mongodb.com/company/careers/how-to-avoid-recruiting-scams', 'notes': 'Investigation found a single job-like item and a link to a careers guidance page. No structured API pagination discovered in payload.'}
LISTING = {'items_selector': 'div[class~="relative"][class~="z-[9999]"][class~="w-full"]', 'title_selector': 'a[class~="css-6prp03"]', 'url_selector': 'a[class~="css-6prp03"]', 'json_items_path': [], 'items_path': [], 'title_path': [], 'url_path': [], 'location_path': [], 'date_posted_path': [], 'employment_type_path': [], 'work_type_path': [], 'salary_range_path': [], 'notes': 'Use structural extraction. Prefer JSON paths when API returns JSON; otherwise use CSS selectors for HTML. Only selectors/paths present in investigation are used.', 'job_selector': 'div[class~="relative"][class~="z-[9999]"][class~="w-full"]', 'listing_selector': 'div[class~="relative"][class~="z-[9999]"][class~="w-full"]'}
DETAIL = {'description_path': ['description'], 'url_path': [], 'detail_url_path': [], 'apply_url_path': [], 'other_fields': {}, 'notes': "Detail extraction relies on 'description' JSON path if API returns job detail JSON. No HTML detail selectors provided in investigation."}
FILTERS = {'notes': 'No filter parameters discovered in investigation payload.'}
...
LISTING_SELECTOR_TEXT = 'div[class~="relative"][class~="z-[9999]"][class~="w-full"]'
...
def extract_jobs_from_page(html: str, current_url: str, page_index: int):
    soup = _bs(html)
    items = soup.select('div[class~="relative"][class~="z-[9999]"][class~="w-full"]')
    ...
    for item in items:
        title_node = item.select_one('a[class~="css-6prp03"]')
        url_node = item.select_one('a[class~="css-6prp03"]')
...
def iter_jobs():
    ...
    for index in range(max_pages):
        ...
        page_html, rendered_url = fetch_rendered_html(page_url, wait_selector='div[class~="relative"][class~="z-[9999]"][class~="w-full"]')
        yielded_any = False
