...
ENTRY_URL = 'https://simplyhired.co.in/careers'
DEFAULT_OUTPUT = '\\data\\jobs.jsonl'
PAGINATION = {'notes': 'No pagination detected in investigation payload; leave null to avoid inferring behavior.'}
LISTING = {'items_selector': 'span[class~="cf-footer-item"][class~="sm:block"][class~="sm:mb-1"]', 'title_selector': 'a[id="brand_link"]', 'url_selector': 'a[id="brand_link"]', 'json_items_path': [], 'items_json_path': [], 'title_path': [], 'url_path': [], 'job_selector': 'span[class~="cf-footer-item"][class~="sm:block"][class~="sm:mb-1"]', 'listing_selector': 'span[class~="cf-footer-item"][class~="sm:block"][class~="sm:mb-1"]', 'notes': 'Use static HTML parsing (BeautifulSoup) per investigation recommendation. Keep selectors exactly as provided; do not infer new selectors.'}
DETAIL = {'description_path': ['description'], 'url_path': ['url'], 'other_fields': {}, 'notes': 'No structural detail selectors were provided; only JSON paths from the investigation are preserved. Because the investigation recommended BeautifulSoup, the scraper should fetch the entry_url HTML and parse listings with the CSS selectors above. Do not attempt Playwright rendering or add new selectors.'}
FILTERS = {'notes': 'No filter configuration present in investigation payload.'}
...
LISTING_SELECTOR_TEXT = 'span[class~="cf-footer-item"][class~="sm:block"][class~="sm:mb-1"]'
...
def extract_jobs_from_page(html: str, current_url: str, page_index: int):
    soup = _bs(html)
    items = soup.select('span[class~="cf-footer-item"][class~="sm:block"][class~="sm:mb-1"]')
    ...
    for item in items:
        title_node = item.select_one('a[id="brand_link"]')
        url_node = item.select_one('a[id="brand_link"]')
...
def iter_jobs():
    ...
    for index in range(max_pages):
        ...
        page_html, rendered_url = fetch_rendered_html(page_url, wait_selector='span[class~="cf-footer-item"][class~="sm:block"][class~="sm:mb-1"]')
        yielded_any = False
