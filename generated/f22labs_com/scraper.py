...
ENTRY_URL = 'https://f22labs.zohorecruit.in/jobs/Careers'
DEFAULT_OUTPUT = 'jobs.jsonl'
PAGINATION = {'type': 'link', 'notes': 'Investigation pagination provided as external blog link; no SPA pagination selector was provided in payload, so pagination extraction is disabled (null).', 'text': 'blog', 'href': 'https://www.f22labs.com/blogs/?category=Headless%20Shopify&page=1', 'rel': ['noopener', 'noreferrer']}
LISTING = {'items_selector': 'a[href]', 'title_selector': 'a', 'url_selector': 'a[href]', 'location_selector': 'p[class~="filter-subhead"][class~="cw-bw"]', 'date_posted_text_selector': 'span[class~="search-date-type"]', 'employment_type_selector': 'div[class~="cw-filter-joblist-right"]', 'json_items_path': [[], ['founders', 0], ['brand'], ['mainEntityOfPage', 0]], 'notes': 'Use SPA rendering to load the careers page. Prefer structural extraction using provided CSS selectors and json_items_path. Do not infer additional selectors or URLs.', 'job_selector': 'div[class~="cw-filter-joblist"]', 'listing_selector': 'div[class~="cw-filter-joblist"]'}
DETAIL = {'url_path': ['url'], 'description_path': ['description'], 'detail_selector': 'p[class~="filter-subhead"][class~="cw-bw"]', 'notes': 'Detail extraction should use the provided JSON paths when available. For HTML detail fallback, use the provided CSS detail_selector. No regex or inferred selectors.', 'description_selector': 'p[class~="filter-subhead"][class~="cw-bw"]'}
FILTERS = {'notes': 'No filter selectors or parameters provided in investigation payload; keep filters null.'}
...
def fetch_detail_fields(url: str) -> tuple[Any, Any, Any, Any, Any]:
    detail_html, detail_url = fetch_rendered_html(url, wait_selector='p[class~="filter-subhead"][class~="cw-bw"]')
    detail_soup = _bs(detail_html)
    description_node = detail_soup.select_one('p[class~="filter-subhead"][class~="cw-bw"]') if 'p[class~="filter-subhead"][class~="cw-bw"]' else None
    date_node = detail_soup.select_one('span[class~="search-date-type"]') if 'span[class~="search-date-type"]' else None
    employment_node = detail_soup.select_one('div[class~="cw-filter-joblist-right"]') if 'div[class~="cw-filter-joblist-right"]' else None
...
def extract_jobs_from_page(html: str, current_url: str, page_index: int):
    ...
    for item in items:
        title_node = item.select_one('a') if 'a' else None
        url_node = item.select_one('a[href]') if 'a[href]' else None
        location_node = item.select_one('p[class~="filter-subhead"][class~="cw-bw"]') if 'p[class~="filter-subhead"][class~="cw-bw"]' else None
        description_node = item.select_one('p[class~="filter-subhead"][class~="cw-bw"]') if 'p[class~="filter-subhead"][class~="cw-bw"]' else None
        date_node = item.select_one('span[class~="search-date-type"]') if 'span[class~="search-date-type"]' else None
        employment_node = item.select_one('div[class~="cw-filter-joblist-right"]') if 'div[class~="cw-filter-joblist-right"]' else None
...
def iter_jobs():
    ...
    for index in range(max_pages):
        ...
        page_html, rendered_url = fetch_rendered_html(page_url, wait_selector='a[href]')
        yielded_any = False
